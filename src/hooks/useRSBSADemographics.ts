...
export const useRSBSADemographics = (records: any[]): RSBSADemographicsData => {
  ...
  const demographics = useMemo(() => {
    ...
    const cropDistribution: CropDistribution[] = cropFields.map(
      ({ key, label, color }) => {
    ...
    records.forEach((r) => {
      // Categorize each farmer into ONE primary ownership type to avoid double-counting
      if (r.ownershipType?.registeredOwner) {
        registered++;
      } else if (r.ownershipType?.tenant) {
        tenantCount++;
      } else if (r.ownershipType?.lessee) {
        lesseeCount++;
      }
    });
    ...
    const ownershipBreakdown: OwnershipBreakdown[] = [
      {
        ...
        count: registered,
        percentage: pct(registered, total),
        color: "#16a34a",
      },
      {
        type: "Tenant",
        count: tenantCount,
        percentage: pct(tenantCount, total),
        color: "#f59e0b",
      },
      {
        type: "Lessee",
        count: lesseeCount,
        percentage: pct(lesseeCount, total),
        color: "#ef4444",
      },
    ];
    ...
    const genderBreakdown: GenderBreakdown[] = [
      {
        gender: "Male",
        count: male,
        percentage: pct(male, total),
        color: "#3b82f6",
      },
      {
        gender: "Female",
        count: female,
        percentage: pct(female, total),
        color: "#ec4899",
      },
      ...(otherGender > 0
        ? [
            {
              gender: "Other",
              count: otherGender,
              percentage: pct(otherGender, total),
              color: "#6b7280",
            },
          ]
        : []),
