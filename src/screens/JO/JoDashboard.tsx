      ...
      try {
        ...
        if (!response.error && response.data) {
          ...
          const current = response.data.find(
            (s: AvailableSeason) => s.status === "active",
          );
          if (current) {
          ...
          const defaultSeason =
            month >= 4 && month <= 9 ? `wet_${year}` : `dry_${year}`;
          setAvailableSeasons([
            {
              id: 0,
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [statsResponse] = await Promise.all([
          hasValidAllocationId
            ? getDashboardStats(parsedAllocationId, true)
            : getDashboardStats(selectedSeasonLabel),
          hasValidAllocationId
            ? getMonthlyTrends(parsedAllocationId, true)
            : getMonthlyTrends(selectedSeasonLabel),
          hasValidAllocationId
            ? getRecentActivity(5, parsedAllocationId, true)
            : getRecentActivity(5, selectedSeasonLabel),
        ]);
        ...
        <div className="jo-executive-dashboard">
          ...
          <div className="executive-header">
            <div className="header-left">
              <h1 className="executive-title">Dashboard</h1>
              <p className="executive-subtitle">
                {formatSeason(selectedSeasonLabel)} • Last updated:{" "}
