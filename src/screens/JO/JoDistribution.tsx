...
const JoDistribution: React.FC = () => {
  ...
  const [viewDetailsModal, setViewDetailsModal] =
      ...
      if (!response.error) {
        ...
        if (data.length > 0) {
          const mostRecent = data.sort(
            (a: RegionalAllocation, b: RegionalAllocation) =>
  ...
  const fetchDistributions = async () => {
    // Fetch distribution records based on selected season
    ...
    try {
      ...
      if (!response.error) {
        ...
        const filteredData = allRecords.filter(
          (record: DistributionRecord) => record.season === selectedSeason,
        );
  ...
  const calculateCompletionStats = (
    ...
    const uniqueFarmers = new Set(
      records.map((r: DistributionRecord) => r.rsbsa_number),
    ).size;
    ...
    setCompletionStats({
        ...
        percentage:
          totalFertilizerAllocated > 0
            ? Math.min(
                100,
                Math.round(
                  (totalFertilizerDistributed / totalFertilizerAllocated) * 100,
                ),
              )
            : 0,
      },
      seeds: {
        ...
        percentage:
          totalSeedAllocated > 0
            ? Math.min(
                100,
                Math.round((totalSeedDistributed / totalSeedAllocated) * 100),
              )
            : 0,
      },
      farmers: {
        ...
        percentage: Math.min(
          100,
          Math.round((uniqueFarmers / estimatedTotalFarmers) * 100),
        ),
      },
    });
  };
  ...
  const generateTimelineData = (
    ...
    records.forEach((record: DistributionRecord) => {
      ...
      const bagsDistributed =
        (record.fertilizer_bags_given || 0) + (record.seed_kg_given || 0) / 10;
  ...
  const generateBarangayPerformance = (records: DistributionRecord[]) => {
    ...
    const performances: BarangayPerformance[] = barangays.map((barangay) => {
      ...
      let status: "complete" | "pending" | "shortage" | "logistics" =
        "complete";

      if (percentage < 60) {
        const statusOptions: ("pending" | "shortage" | "logistics")[] = [
          "pending",
          "shortage",
          "logistics",
        ];
    ...
    const needAttention = performances
    ...
    return (
      <div className="jo-dist-donut-card">
        ...
        <div className="jo-dist-donut-stats">
          <span className="jo-dist-donut-value">
            {distributed.toLocaleString()}/{allocated.toLocaleString()}
          </span>
    ...
    <div className="distribution-container has-mobile-sidebar">
      ...
      <div className="main-content distribution-main-content">
        ...
        <div className="content-header distribution-content-header">
          ...
          <p>
            View all approved farmer distributions organized by regional
            allocation
          </p>
        </div>
                  ...
                  <>
                    ...
                    {allocations.map((alloc) => (
                      <option key={alloc.id} value={alloc.season}>
                        {alloc.season.replace("_", " ").toUpperCase()} -{" "}
          ...
          {showCompletionReport && completionStats && (
            <div className="jo-dist-completion-report">
              ...
              <div className="jo-dist-report-header">
                ...
                <p className="jo-dist-report-season">
                  Season: {selectedSeason.replace("_", " ").toUpperCase()}
                </p>
              </div>
              ...
              <div className="jo-dist-timeline-section">
                <h4 className="jo-dist-section-title">
                  📈 DISTRIBUTION TIMELINE
                </h4>
                ...
                <div className="jo-dist-timeline-summary">
                  {timelineData.map((item, idx) => (
                    <div key={idx} className="jo-dist-timeline-item">
                      ...
                      <span className="jo-dist-timeline-value">
                        +{item.distributed} bags
                      </span>
                    </div>
                  ))}
                </div>
                    ...
                    <div className="jo-dist-performance-list">
                      {topPerforming.map((item, idx) => (
                        <div key={idx} className="jo-dist-performance-item">
                          <div className="jo-dist-performance-rank">
                            {idx + 1}
                          </div>
                          <div className="jo-dist-performance-info">
                            <span className="jo-dist-barangay-name">
                              {item.barangay}
                            </span>
                          ...
                          <span className="jo-dist-percentage jo-dist-success">
                            {item.percentage}%
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="jo-dist-performance-card jo-dist-needs-attention">
                    ...
                    <div className="jo-dist-performance-list">
                      {needsAttention.map((item, idx) => (
                        <div key={idx} className="jo-dist-performance-item">
                          <div className="jo-dist-performance-rank jo-dist-warning">
                            {idx + 1}
                          </div>
                          <div className="jo-dist-performance-info">
                            <span className="jo-dist-barangay-name">
                            ...
                            <span
                              className={`jo-dist-status-badge jo-dist-${item.status}`}
                            >
                              {item.status}
                            </span>
                          </div>
                          <span className="jo-dist-percentage jo-dist-warning">
                            {item.percentage}%
                          </span>
                        </div>
                      ))}
              ...
              <tbody>
                {distributions.length === 0 ? (
                  ...
                  distributions.map((dist) => {
                    const fertilizers = parseFertilizerTypes(
                      dist.fertilizer_type,
                    );
                    ...
                    return (
                      <tr key={dist.id}>
                        <td>
                          <div>
                            <strong>{dist.farmer_name}</strong>
                          </div>
                        ...
                        <div className="distribution-modal-body">
                          {requestCount > 0 && (
                            ...
                            >
                              ...
                              {requestCount} existing farmer request(s). Changes
                              may affect these requests.
                            </div>
                          )}
                ...
                <div className="distribution-modal-body">
                    ...
                    <div className="distribution-detail-grid">
                      <div className="distribution-detail-item">
                        ...
                        <span className="detail-value">
                          {viewDetailsModal.farmer_name}
                        </span>
                      </div>
                      <div className="distribution-detail-item">
                        <span className="detail-label">RSBSA Number:</span>
                        <span className="detail-value">
                          {viewDetailsModal.rsbsa_number}
                        </span>
                      </div>
                      <div className="distribution-detail-item">
                        ...
                        <span className="detail-value">
                          {new Date(
                            viewDetailsModal.distribution_date,
                  ...
                  <div className="distribution-detail-section">
                    ...
                    <div className="distribution-breakdown-list">
                      {parseFertilizerTypes(viewDetailsModal.fertilizer_type)
                        .length > 0 ? (
                        parseFertilizerTypes(
                          viewDetailsModal.fertilizer_type,
                        ).map((item, idx) => (
                          <div
                            key={idx}
                            className="distribution-breakdown-item"
                          >
                      ...
                      {parseFertilizerTypes(viewDetailsModal.fertilizer_type)
                        ...
                        <div className="distribution-breakdown-total">
                          ...
                          <span className="breakdown-amount">
                            <strong>
                              {viewDetailsModal.fertilizer_bags_given || 0} bags
                            </strong>
                          </span>
                        </div>
                      )}
                  ...
                  <div className="distribution-detail-section">
                    ...
                    <div className="distribution-breakdown-list">
                      {parseSeedTypes(viewDetailsModal.seed_type).length > 0 ? (
                        parseSeedTypes(viewDetailsModal.seed_type).map(
                          (item, idx) => (
                            <div
                              key={idx}
                              className="distribution-breakdown-item"
                      ...
                      {parseSeedTypes(viewDetailsModal.seed_type).length >
                        0 && (
                        <div className="distribution-breakdown-total">
                          <span className="breakdown-type">
                            <strong>Total Seeds</strong>
                          </span>
                          <span className="breakdown-amount">
                            <strong>
                              {Number(
                                viewDetailsModal.seed_kg_given || 0,
                              ).toFixed(2)}{" "}
                              kg
                            </strong>
                          </span>
                  ...
                  {viewDetailsModal.verification_notes && (
                    <div className="distribution-detail-section">
                      <h4>Notes</h4>
                      <p className="distribution-notes-text">
                        {viewDetailsModal.verification_notes}
                      </p>
                    </div>
                  )}
