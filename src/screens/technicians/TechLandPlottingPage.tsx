...
>>>>>>> 3405086e1de361b58526f3720d311f5faef5da57
  ...
  const handleSaveAttributes = async () => {
    setIsSaving(true);
    try {
      // --- Area validation: check drawn polygon area vs target parcel area ---
      const targetAreaHa = parseFloat(
        currentParcel?.total_farm_area_ha ||
          currentParcel?.farm_size ||
          currentParcel?.total_farm_area ||
          "0",
      );
      ...
      if (targetAreaHa > 0) {
        for (const shape of shapes) {
          ...
          if (geo?.geometry?.type === "Polygon") {
            // Compute actual geodesic area in m²
            const latlngs = (shape.layer.getLatLngs?.() || [])[0] as
              | L.LatLng[]
              | undefined;
            if (latlngs && latlngs.length >= 3) {
              const drawnAreaM2 = (L as any).GeometryUtil.geodesicArea(latlngs);
              const drawnAreaHa = drawnAreaM2 / 10000;
              const tolerance = 1.05; // 5% tolerance
              if (drawnAreaHa > targetAreaHa * tolerance) {
                const overPct = (
                  (drawnAreaHa / targetAreaHa - 1) *
                  100
                ).toFixed(1);
                setToast({
                  message: `Drawn area (${drawnAreaHa.toFixed(4)} ha) exceeds the target parcel area (${targetAreaHa} ha) by ${overPct}%. Please redraw the polygon within the allowed area.`,
                  type: "error",
                });
    ...
    try {
      ...
      if (parcelIndex !== undefined && farmParcels) {
        ...
        if (parcel) {
          ...
          setLandAttributes((prev) => ({
            ...prev,
            ffrs_id: data.referenceNumber || data.FFRS_CODE || "",
            firstName: data.firstName || "",
            middleName: data.middleName || "",
            surname: data.lastName || data.surname || "",
            gender: data.gender || "Male",
          ...
          if (selected) {
            console.log(
              "📝 Setting landAttributes from selected shape:",
              selected.properties,
            );
            setLandAttributes((prev) => ({ ...prev, ...selected.properties }));
          }
...
>>>>>>> 3405086e1de361b58526f3720d311f5faef5da57
  ...
  const barangayForMap = useMemo(() => {
    ...
    console.log(
      "✅ useMemo barangayForMap - FINAL selected value:",
      `"${value}"`,
    );
    return value || "";
  }, [
  ...
  function getDisplayName() {
    ...
    console.log(
      "🔍 getDisplayName - selectedShape?.properties:",
      selectedShape?.properties,
    );
