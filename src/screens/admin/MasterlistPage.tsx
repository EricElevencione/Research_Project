...
import type { UnifiedParcel } from "../../components/FarmerProfile/FarmerProfileDisplay";

// ─────────────────────────────────────────────
// MASTERLIST — Universal "Find Anyone" page (Admin Version)
...
const Masterlist: React.FC = () => {
  ...
  const getOwnershipClass = (record: RSBSARecord) => {
    ...
    if (f.category === "registeredOwner" || f.owner)
      return "admin-masterlist-ownership-owner";
    if (f.tenant && f.lessee) return "admin-masterlist-ownership-mixed";
    if (f.tenant) return "admin-masterlist-ownership-tenant";
    if (f.lessee) return "admin-masterlist-ownership-lessee";
    if (f.tenantLessee) return "admin-masterlist-ownership-tenant";
    return "admin-masterlist-ownership-unknown";
  };
  ...
  return (
    <div className="admin-masterlist-page-container">
      <div className="admin-masterlist-page has-mobile-sidebar">
        <AdminSidebar
          sidebarOpen={sidebarOpen}
          setSidebarOpen={setSidebarOpen}
        />

        <div className="admin-masterlist-main-content">
          {/* Mobile header */}
          ...
          {/* Page header */}
          <div className="admin-masterlist-dashboard-header">
            <div>
              <h1 className="admin-masterlist-page-title">Masterlist</h1>
              <p className="admin-masterlist-page-subtitle">
          ...
          {/* ── Summary Cards ─────────────────────────────────────────────── */}
          {!loading && !error && (
            <div className="admin-masterlist-status-cards">
              <div className="admin-masterlist-status-card admin-masterlist-card-total">
                <div className="admin-masterlist-card-icon">👥</div>
                <div className="admin-masterlist-card-info">
                  <span className="admin-masterlist-card-count">
                    {statusCounts.total}
                  </span>
                  <span className="admin-masterlist-card-label">
                    Total Registered
                  </span>
                </div>
              </div>
              <div className="admin-masterlist-status-card admin-masterlist-card-active">
                <div className="admin-masterlist-card-icon">✅</div>
                <div className="admin-masterlist-card-info">
                  <span className="admin-masterlist-card-count">
                    {statusCounts.active}
              ...
              {statusCounts.noParcels > 0 && (
                <div
                  className="admin-masterlist-status-card admin-masterlist-card-inactive"
                ...
                >
                  <div className="admin-masterlist-card-icon">⚠️</div>
                  <div className="admin-masterlist-card-info">
                    <span className="admin-masterlist-card-count">
                      {statusCounts.noParcels}
                    </span>
                    <span className="admin-masterlist-card-label">No Parcels</span>
                  </div>
                </div>
              )}
          ...
          <div className="admin-masterlist-content-card">
            {/* ── Filters ──────────────────────────────────────────────── */}
            <div className="admin-masterlist-filters-section">
              {/* Row 1 — Search */}
              <div className="admin-masterlist-filters-row-1">
                <div className="admin-masterlist-search-filter">
                  <input
                    type="text"
                    placeholder="Search name, FFRS code, farm barangay, landowner, parcel no..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="admin-masterlist-search-input"
                  />
                </div>
              </div>
              {/* Row 2 — Dropdowns */}
              <div className="admin-masterlist-filters-row-2">
                <div className="admin-masterlist-status-filter">
                  <select
                    value={selectedRole}
                    onChange={(e) => setSelectedRole(e.target.value)}
                    className="admin-masterlist-status-select"
                  >
                ...
                >
                  ...
                  {landSizeFilter.trim() !== "" && (
                    <>
                      ...
                      <button
                        type="button"
                        className="admin-masterlist-land-clear-btn"
            ...
            {!loading && !error && (
              <div className="admin-masterlist-table-meta">
                {!isDefaultSortConfig && (
                  <button
                    type="button"
                    className="admin-masterlist-sort-btn"
            ...
            {/* ── Bulk toolbar ─────────────────────────────────────────── */}
            {!loading && !error && (
              <div className="admin-masterlist-bulk-toolbar">
                {selectedRecordIds.size > 0 && (
                  <span className="admin-masterlist-bulk-count">
                    {selectedRecordIds.size} record
                    {selectedRecordIds.size === 1 ? "" : "s"} selected
                  </span>
                )}
                <div className="admin-masterlist-bulk-actions">
                  <div
                    className="admin-masterlist-bulk-export-wrap"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button
                      className="admin-masterlist-bulk-btn"
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    {showPrintMasterlistModal && (
                      <div className="admin-masterlist-bulk-menu">
                        <div
                          style={{
                        ...
                        <button
                          className="admin-masterlist-quick-item"
                          onClick={() => {
                  ...
                  {selectedRecordIds.size > 0 && (
                    <button
                      className="admin-masterlist-bulk-btn admin-masterlist-bulk-btn-clear"
            ...
            {/* ── Table ────────────────────────────────────────────────── */}
            <div className="admin-masterlist-table-container">
              <table className="admin-masterlist-farmers-table">
                <thead>
                  <tr>
                    <th className="admin-masterlist-checkbox-col">
                      <input
                        type="checkbox"
                        className="admin-masterlist-header-checkbox"
                    ...
                    <th>
                      <button
                        className={`admin-masterlist-sort-btn ${isSortActive("farmerName") ? "is-active" : ""}`}
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <th>
                      <button
                        className={`admin-masterlist-sort-btn ${isSortActive("parcelArea") ? "is-active" : ""}`}
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <th>
                      <button
                        className={`admin-masterlist-sort-btn ${isSortActive("status") ? "is-active" : ""}`}
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <th>
                      <button
                        className={`admin-masterlist-sort-btn ${isSortActive("dateSubmitted") ? "is-active" : ""}`}
                        onClick={(e) => {
                          e.stopPropagation();
                ...
                <tbody>
                  {loading && (
                    <tr>
                      <td
                        ...
                        className="admin-masterlist-loading-cell"
                      >
                        Loading...
                      </td>
                    </tr>
                  ...
                  {error && !loading && (
                    <tr>
                      <td
                        colSpan={VISIBLE_COLUMN_COUNT}
                        className="admin-masterlist-error-cell"
                      >
                        Error: {error}
                      </td>
                    </tr>
                    ...
                    filteredRecords.map((record) => {
                      ...
                      return (
                        <React.Fragment key={record.id}>
                          {/* ── Main owner row ── */}
                          <tr
                            className={`admin-masterlist-table-row${missingRecordWarning ? " admin-row--flagged" : ""}`}
                            onClick={() =>
                              fetchFarmerDetails(record.id, record)
                            }
                          >
                            <td className="admin-masterlist-checkbox-col">
                              <input
                                type="checkbox"
                                className="admin-masterlist-row-checkbox"
                            ...
                            <td>
                              <div className="admin-masterlist-farmer-cell">
                                <div className="admin-masterlist-farmer-avatar">
                                  {getFarmerInitials(record.farmerName)}
                                </div>
                                <div className="admin-masterlist-farmer-meta">
                                  <span className="admin-masterlist-farmer-name">
                                    {record.farmerName}
                                    {hasIdleParcel && (
                                      <span
                                  ...
                                  <span className="admin-masterlist-farmer-ref">
                                  ...
                                  {hasTenants && (
                                    <button
                                      className="admin-masterlist-tenant-badge"
                                      onClick={(e) => {
                                        e.stopPropagation();
                            ...
                            <td>
                              <span className="admin-masterlist-cultivation-text">
                                {record.farmBarangay || "—"}
                              </span>
                            </td>
                            <td>
                              <div className="admin-masterlist-parcel-cell">
                                <span className="admin-masterlist-parcel-count">
                                  {record.parcelCount} parcel
                                  {record.parcelCount === 1 ? "" : "s"}
                                </span>
                                <span className="admin-masterlist-parcel-area">
                            ...
                            <td>
                              <span
                                className="admin-masterlist-record-status"
                                style={
                            ...
                            <td>
                              <span className="admin-masterlist-date">
                                {formatDate(record.dateSubmitted)}
                              </span>
                            </td>
                            <td className="admin-masterlist-actions-cell">
                              <div
                                className="admin-masterlist-quick-actions"
                                onClick={(e) => e.stopPropagation()}
                              >
                                <button
                                  className="admin-masterlist-view-btn"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setOpenMenuId((p) =>
                                ...
                                {openMenuId === record.id && (
                                  <div className="admin-masterlist-quick-menu">
                                    <button
                                      className="admin-masterlist-quick-item"
                                      onClick={() => {
                                        fetchFarmerDetails(record.id, record);
                          ...
                          {hasTenants && isExpanded && (
                            <tr className="admin-masterlist-tenant-subrow">
                              <td />
                              <td colSpan={VISIBLE_COLUMN_COUNT - 1}>
                                <div className="admin-masterlist-tenant-panel">
                                  <div className="admin-masterlist-tenant-panel-header">
                                    <span className="admin-masterlist-tenant-panel-title">
                                      🌾 Farmers under{" "}
                                      <strong>{record.farmerName}</strong>
                                    </span>
                                    <span className="admin-masterlist-tenant-panel-note">
                                      Showing RSBSA-registered tenants/lessees
                                      only
                                    </span>
                                  </div>
                                  <table className="admin-masterlist-tenant-table">
                                    <thead>
                                      <tr>
                                        <th>Farmer</th>
                                        <th>FFRS Code</th>
                                    ...
                                    <tbody>
                                      {tenants.map((t) => (
                                        <tr
                                          key={t.id}
                                          className="admin-masterlist-tenant-row"
                                          onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        >
                                          <td>
                                            <div className="admin-masterlist-tenant-farmer-cell">
                                              <div className="admin-masterlist-tenant-avatar">
                                                {getFarmerInitials(
                                                  t.farmerName,
                                                )}
                                              </div>
                                              ...
                                              >
                                                <span className="admin-masterlist-tenant-name">
                                                  {t.farmerName}
                                                </span>
                                                {t.hasNoLandOwner && (
                                                  <span
                                          ...
                                          <td className="admin-masterlist-tenant-ref">
                                            {t.referenceNumber || "—"}
                                          </td>
                                          <td className="admin-masterlist-tenant-barangay">
                                            {t.farmBarangay || "—"}
                                          </td>
                                          <td>
                                            <div className="admin-masterlist-parcel-cell">
                                          ...
                                          <td>
                                            <span
                                              className={`admin-masterlist-ownership-pill ${getOwnershipClass(t)}`}
                                            >
                                              {getTenantRoleLabel(t)}
                                            </span>
                                          </td>
                                          <td>
                                            <span className="admin-masterlist-record-status">
                  ...
                  {!loading && !error && filteredRecords.length === 0 && (
                    <tr>
                      <td
                        colSpan={VISIBLE_COLUMN_COUNT}
                        className="admin-masterlist-empty-cell"
                      >
        ...
        {updateNotification.show && (
          <div
            className={`admin-masterlist-update-toast ${updateNotification.type}`}
            role="status"
            aria-live="polite"
          >
            <span className="admin-masterlist-update-toast-message">
              {updateNotification.message}
            </span>
            <button
              className="admin-masterlist-update-toast-close"
              onClick={() =>
                setUpdateNotification((p) => ({ ...p, show: false }))
              }
              aria-label="Close"
