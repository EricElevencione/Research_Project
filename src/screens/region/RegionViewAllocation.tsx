...
const RegionViewAllocation: React.FC = () => {
  ...
  const gaugeData = useMemo(() => {
    if (!allocation) return { fertilizers: [], seeds: [] };

    const fertilizers = FERTILIZER_FIELD_MAPS.map((fertilizer) => {
      const allocated = Number(allocation[fertilizer.allocationField]) || 0;
    ...
    const seeds = SEED_FIELD_MAPS.map((seed) => {
      const allocated = Number(allocation[seed.allocationField]) || 0;
      const requested = requests.reduce(
