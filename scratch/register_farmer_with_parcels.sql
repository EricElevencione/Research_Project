...
BEGIN
    ...
    -- Step 1: Insert or Update rsbsa_submission
    IF v_self_land_owner_id IS NOT NULL THEN
        -- Reusing existing landowner submission, augment it as a farmer
        ...
        SET 
            ...
            "PARCEL AREA" = COALESCE((p_data->'farmlandParcels'->0->>'totalFarmAreaHa')::DECIMAL, "PARCEL AREA"),
    ...
    ELSE
        -- Normal path: insert a new submission
    ...
    RAISE NOTICE 'Processing parcels for submission % (%)', v_submission_id, v_farmer_name;
    ...
    LOOP
        ...
        IF v_has_existing_parcel_ref THEN
            ...
            IF v_history_record.id IS NOT NULL THEN
                ...
                -- ONLY close history if the previous holder is DIFFERENT from the submission (actual transfer)
        ...
        -- If we are reusing the existing landowner's parcel record (same owner and parcel), we UPDATE instead of INSERT.
        IF v_has_existing_parcel_ref AND v_prev_farmer_id = v_submission_id THEN
            ...
            RAISE NOTICE 'Updated existing farm_parcel id: % for owner %', v_farm_parcel_id, v_submission_id;
        ...
        ELSE
            ...
            -- Only deactivate old parcel if this is an actual transfer of ownership to a different person
            IF v_parcel_is_registered_owner AND v_prev_farmer_id IS NOT NULL AND v_prev_farmer_id != v_submission_id THEN
                ...
                IF v_remaining_count = 0 THEN
                    ...
                    RAISE NOTICE 'Marked previous owner % as inactive', v_prev_farmer_id;
                END IF;
            END IF;
        END IF;
    END LOOP;
