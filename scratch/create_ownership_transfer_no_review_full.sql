CREATE OR REPLACE FUNCTION public.create_ownership_transfer_no_review(p_transfer_mode text, p_from_farmer_id bigint, p_to_farmer_id bigint, p_source_role text, p_area_mode text, p_area_requested_ha numeric, p_area_available_ha numeric, p_transfer_reason text, p_transfer_date date, p_is_deceased_confirmed boolean DEFAULT false, p_items jsonb DEFAULT '[]'::jsonb, p_proofs jsonb DEFAULT '[]'::jsonb)
 RETURNS bigint
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
begin
  ...
  loop
    ...
    if v_transfer_area >= (v_parcel_area - 0.0001) then
      ...
      -- Update existing tenants/lessees of this physical parcel to point to the new owner (recipient)
      update rsbsa_farm_parcels
      set
        tenant_land_owner_id = case when ownership_type_tenant = true then p_to_farmer_id else tenant_land_owner_id end,
        tenant_land_owner_name = case when ownership_type_tenant = true then v_to_name else tenant_land_owner_name end,
        lessee_land_owner_id = case when ownership_type_lessee = true then p_to_farmer_id else lessee_land_owner_id end,
        lessee_land_owner_name = case when ownership_type_lessee = true then v_to_name else lessee_land_owner_name end,
        updated_at = now()
      where (parcel_number IS NOT NULL AND v_parcel_number IS NOT NULL AND lower(trim(parcel_number)) = lower(trim(v_parcel_number)))
        and (tenant_land_owner_id = p_from_farmer_id or lessee_land_owner_id = p_from_farmer_id);

      -- Update current land history records of those tenants to point to the new owner
      update land_history
      set
        land_owner_id = p_to_farmer_id,
        land_owner_name = v_to_name,
        land_owner_ffrs_code = v_to_ffrs,
        change_reason = 'Landowner transfer to ' || v_to_name,
        updated_at = now()
      where (parcel_number IS NOT NULL AND v_parcel_number IS NOT NULL AND lower(trim(parcel_number)) = lower(trim(v_parcel_number)))
        and (land_owner_id = p_from_farmer_id)
        and (is_tenant = true or is_lessee = true)
        and is_current = true;

      -- Update matching land plots to point to the new owner (recipient)
      update public.land_plots
      set
        farmer_id = p_to_farmer_id,
        ffrs_id = v_to_ffrs,
        first_name = (select "FIRST NAME" from rsbsa_submission where id = p_to_farmer_id),
        middle_name = (select "MIDDLE NAME" from rsbsa_submission where id = p_to_farmer_id),
        surname = (select "LAST NAME" from rsbsa_submission where id = p_to_farmer_id),
        ext_name = (select "EXT NAME" from rsbsa_submission where id = p_to_farmer_id),
        updated_at = now()
      where (parcel_number IS NOT NULL AND v_parcel_number IS NOT NULL AND lower(trim(parcel_number)) = lower(trim(v_parcel_number)))
    ...
    -- PARTIAL transfer: reduce donor parcel + create recipient parcel
      ...
      set
        ...
        total_farm_area_ha = v_remaining_area,
        transferred_area_ha = v_transfer_area,
