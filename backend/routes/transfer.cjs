...
const assertTransferMode = (mode) => mode === 'voluntary' || mode === 'inheritance';
...
const ensureSingleCurrentHistory = async (
    ...
    const agrarianReformBeneficiaryBool = boolFromYesNo(agrarianReformBeneficiary);
...
const refreshFarmerOwnershipSummary = async (client, farmerId) => {
...
router.post('/confirm', async (req, res) => {
    ...
    try {
        ...
        for (let index = 0; index < transferItems.length; index += 1) {
            ...
            if (!donorParcel.submissionId || donorParcel.submissionId !== donorFarmer.id) {
                throw new Error(
            ...
            if (donorParcel.totalFarmAreaHa <= AREA_EPSILON) {
                throw new Error(
                    `Parcel ${farmParcelId} has no transferable area.`
                );
            }
            ...
            totalTransferredAreaHa = Math.round(
                (totalTransferredAreaHa + requestedAreaHa) * 100
            ) / 100;
        }

        const donorSummary = await refreshFarmerOwnershipSummary(client, donorFarmer.id);
