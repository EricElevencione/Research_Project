...
router.get("/allocations", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch allocations", message: error.message });
  }
});
...
router.get("/allocations/:season", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Allocation not found for this season" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch allocation", message: error.message });
  }
});
...
router.post("/allocations", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to save allocation", message: error.message });
  }
});
...
router.delete("/allocations/:id", async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM farmer_requests WHERE season = $1", [season]);

    // Delete the allocation
    await pool.query("DELETE FROM regional_allocations WHERE id = $1", [id]);

    res.json({
      message:
        "Regional allocation and associated requests deleted successfully",
      season: season,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete allocation", message: error.message });
  }
});
  ...
  try {
    const result = await pool.query(
      ...
      [
        ...
        ownership_type,
        num_parcels || 1,
        fertilizer_requested || false,
        seeds_requested || false,
        request_notes,
        created_by,
    ...
    res
      .status(500)
      .json({ error: "Failed to create request", message: error.message });
  }
});
...
router.post("/suggest-alternatives", async (req, res) => {
  ...
  try {
    ...
    if (allocationResult.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No allocation found for this season" });
    }
    ...
    const remainingStock = {
      urea_46_0_0_bags:
        (allocation.urea_46_0_0_bags || 0) - (approved.approved_urea || 0),
      complete_14_14_14_bags:
        (allocation.complete_14_14_14_bags || 0) -
        (approved.approved_complete_14 || 0),
      complete_16_16_16_bags:
        (allocation.complete_16_16_16_bags || 0) -
        (approved.approved_complete_16 || 0),
      ammonium_sulfate_21_0_0_bags:
        (allocation.ammonium_sulfate_21_0_0_bags || 0) -
        (approved.approved_amsul || 0),
      ammonium_phosphate_16_20_0_bags:
        (allocation.ammonium_phosphate_16_20_0_bags || 0) -
        (approved.approved_amph || 0),
      muriate_potash_0_0_60_bags:
        (allocation.muriate_potash_0_0_60_bags || 0) -
        (approved.approved_potash || 0),
      jackpot_kg:
        (allocation.jackpot_kg || 0) - (approved.approved_jackpot || 0),
      ...
      lumping143_kg:
        (allocation.lumping143_kg || 0) - (approved.approved_lumping143 || 0),
    ...
    const farmerRequestData = {
      ...
      requested_ammonium_phosphate_bags:
        request.requested_ammonium_phosphate_bags || 0,
    ...
    console.log(
      `   ✅ Generated ${suggestions.suggestions?.length || 0} alternative suggestions`,
    );
...
router.get("/recommendations/:season", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `   Summary: ${recommendations.summary.critical_issues} critical, ${recommendations.summary.high_priority_issues} high priority`,
    );
...
router.get("/gap-analysis/:season", async (req, res) => {
  ...
  try {
    ...
    const requestedComplete14 =
      parseFloat(requests.requested_complete_14) || 0.01;
    ...
    const totalRiceSeeds =
      parseFloat(requests.requested_jackpot) +
      parseFloat(requests.requested_us88) +
      parseFloat(requests.requested_th82);
    const totalCornSeeds =
      parseFloat(requests.requested_rh9000) +
      parseFloat(requests.requested_lumping143) +
      parseFloat(requests.requested_lp296);
    ...
    const fertilizerGap = {
      urea: {
        ...
        percentage:
          requestedUrea > 0
            ? ((allocation.urea_46_0_0_bags / requestedUrea) * 100).toFixed(1)
            : "0",
      },
      complete_14_14_14: {
        ...
        percentage:
          requestedComplete14 > 0
            ? (
                (allocation.complete_14_14_14_bags / requestedComplete14) *
                100
              ).toFixed(1)
            : "0",
      },
      ammonium_sulfate: {
        ...
        percentage:
          requestedAmSul > 0
            ? (
                (allocation.ammonium_sulfate_21_0_0_bags / requestedAmSul) *
                100
              ).toFixed(1)
            : "0",
      },
      muriate_potash: {
        ...
        percentage:
          requestedPotash > 0
            ? (
                (allocation.muriate_potash_0_0_60_bags / requestedPotash) *
                100
              ).toFixed(1)
            : "0",
      },
    ...
    const seedsGap = {
      rice_seeds: {
        ...
        allocated:
          parseFloat(allocation.jackpot_kg || 0) +
          parseFloat(allocation.us88_kg || 0) +
          parseFloat(allocation.th82_kg || 0),
        gap:
          parseFloat(allocation.jackpot_kg || 0) +
          parseFloat(allocation.us88_kg || 0) +
          parseFloat(allocation.th82_kg || 0) -
      ...
      corn_seeds: {
        ...
        allocated:
          parseFloat(allocation.rh9000_kg || 0) +
          parseFloat(allocation.lumping143_kg || 0) +
          parseFloat(allocation.lp296_kg || 0),
        gap:
          parseFloat(allocation.rh9000_kg || 0) +
          parseFloat(allocation.lumping143_kg || 0) +
          parseFloat(allocation.lp296_kg || 0) -
    ...
    res.json({
      ...
      summary: {
        has_shortages:
          Object.values(fertilizerGap).some((f) => f.gap < 0) ||
          Object.values(seedsGap).some((s) => s.gap < 0),
        prioritization_needed:
          Object.values(fertilizerGap).some((f) => f.percentage < 100) ||
...
router.post("/records", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to record distribution", message: error.message });
  }
});
...
router.put("/records/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update record", message: error.message });
  }
});
...
router.get("/dashboard-stats", async (req, res) => {
  try {
    ...
    const allocation = allocationResult.rows[0] || {
      total_fertilizer_allocated: 0,
      total_seeds_allocated: 0,
    };
    ...
    const fertilizerProgress =
      allocation.total_fertilizer_allocated > 0
        ? Math.round(
            (distributed.total_fertilizer_distributed /
              allocation.total_fertilizer_allocated) *
              100,
          )
        : 0;
    const seedsProgress =
      allocation.total_seeds_allocated > 0
        ? Math.round(
            (distributed.total_seeds_distributed /
              allocation.total_seeds_allocated) *
              100,
          )
        : 0;
    ...
    const totalAllocated =
      parseFloat(allocation.total_fertilizer_allocated || 0) +
      parseFloat(allocation.total_seeds_allocated || 0);
    const totalDistributed =
      parseFloat(distributed.total_fertilizer_distributed || 0) +
      parseFloat(distributed.total_seeds_distributed || 0);
    ...
    const remainingFertilizer = Math.max(
      0,
      parseFloat(allocation.total_fertilizer_allocated || 0) -
        parseFloat(distributed.total_fertilizer_distributed || 0),
    );
    ...
    const requestStatusBreakdown = {
      approved:
        totalRequests > 0
          ? Math.round(
              (parseInt(requests.approved_requests) / totalRequests) * 100,
            )
          : 0,
      pending:
        totalRequests > 0
          ? Math.round(
              (parseInt(requests.pending_requests) / totalRequests) * 100,
            )
          : 0,
      rejected:
        totalRequests > 0
          ? Math.round(
              (parseInt(requests.rejected_requests) / totalRequests) * 100,
            )
          : 0,
      distributed:
        totalRequests > 0
          ? Math.round(
              (parseInt(requests.distributed_requests) / totalRequests) * 100,
            )
          : 0,
    };
    ...
    res.json({
      ...
      coverage: {
        totalBarangays:
          parseInt(barangayCoverage.total_barangays_with_farmers) || 0,
        barangaysWithRequests:
          parseInt(barangayCoverage.barangays_with_requests) || 0,
      },
      processingTime: {
...
router.get("/monthly-trends", async (req, res) => {
  try {
    ...
    res.json({
      ...
      summary: {
        totalFertilizerLast12Months: distributionTrends.reduce(
          (sum, m) => sum + m.fertilizer,
          0,
        ),
        totalSeedsLast12Months: distributionTrends.reduce(
          (sum, m) => sum + m.seeds,
          0,
        ),
        totalDistributionsLast12Months: distributionTrends.reduce(
          (sum, m) => sum + m.count,
          0,
        ),
      },
...
router.get("/barangay-shortages/:season", async (req, res) => {
  ...
  try {
    ...
    const perBarangayUrea = Math.floor(
      (allocation.allocated_urea_bags || 0) / totalBarangays,
    );
    const perBarangayComplete = Math.floor(
      (allocation.allocated_complete_14_bags || 0) / totalBarangays,
    );
    const perBarangaySeeds = Math.floor(
      (allocation.allocated_seeds_kg || 0) / totalBarangays,
    );
    ...
    const shortages = barangayData.rows.map((row) => {
      ...
      const completeGap =
        perBarangayComplete - (row.total_complete_requested || 0);
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch barangay shortages",
        message: error.message,
      });
  }
});
...
router.get("/historical-comparison", async (req, res) => {
  try {
    ...
    console.log(
      `   ✅ Found ${historicalData.length} seasons of historical data`,
    );
